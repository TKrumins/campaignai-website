...
const CHANNELS: Channel[] = [
  ...
  },
  {
    ...
    accent: "#4D9FFF",
    soon: true,
    why: "Streaming apps like Roku, Hulu, and YouTube TV reach cord-cutters with broadcast-style ads and precise targeting.",
    length: "15 or 30 sec",
    format: "Broadcast-style landscape, built to streaming ad specs",
    tips: [
      "Reaches households that no longer watch traditional TV.",
      "We're building toward CTV delivery — it isn't live yet.",
    ],
    pair: "Join the waitlist to be first in line when Connected TV opens.",
  },
  {
    id: "broadcast",
    label: "Broadcast TV",
    icon: Antenna,
    accent: "#7AB8FF",
    why: "Traditional television still commands attention and a sense of credibility that's hard to buy anywhere else.",
    length: "15 or 30 sec",
    format: "Produced to broadcast quality; airtime bought separately",
    tips: [
      "Your video is produced to broadcast-quality standards.",
      "Stations set their own clearance, timing, and legal-review rules — see the note below.",
    ],
    pair: "Confirm station requirements and consult counsel before you air.",
  },
];
    ...
    <section className="bg-white py-20 md:py-28">
      <div className="mx-auto max-w-[1100px] px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-6 md:grid-cols-[300px_minmax(0,1fr)]">
            ...
            {CHANNELS.map((c) => {
              ...
              return (
                <button
                  ...
                  onClick={() => setId(c.id)}
                  aria-pressed={on}
                  className={`group inline-flex items-center gap-3 rounded-xl border px-4 py-3 text-left transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-freedom-blue ${
                ...
                >
                  ...
                  {c.soon && (
                    <span className={`rounded-full px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider ${on ? "bg-pioneer-gold/25 text-pioneer-gold" : "bg-pioneer-gold/15 text-pioneer-gold"}`}>
                      Soon
                    </span>
                  )}
                </button>
          ...
          {/* Detail panel — re-keyed so it settles in on each change */}
          <div key={id} className="vh-settle rounded-3xl border border-gray-200 bg-dawn-frost/50 p-6 sm:p-8">
            <div className="mb-5 flex items-center gap-3">
              ...
              <div>
                ...
                {active.soon && (
                  <span className="text-xs font-bold uppercase tracking-wider text-pioneer-gold">Coming soon</span>
                )}
              </div>
            </div>
